...
| SWD0005 | ❌ Error | The type containing the `[DependencyProperty]` property is not derived from `DependencyObject`, so `GetValue` and `SetValue` are not available | Derive the containing type from `DependencyObject` |
